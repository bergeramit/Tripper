...
public class TrieNode
{
    ...
    private int[] citiesIndex; // city indexes** = for every key word the cities which have this key word
    ...
    protected boolean addCityToKeyWord(int c_index){
        int j=0;
        while(citiesIndex[j] != -1 ) {
            ++j;
            if(j == 512){
                return false;
            }
        }
        citiesIndex[j] = c_index;
        return true;
    }


    //array which have index of the cities with this key word
    protected int[] getCitiesArray(){
        return citiesIndex;
    }
    ...
    public boolean UpdateScore(String keyWord){
        ...
        while(citiesIndex[i]!=-1){
            //save.addKeyToCity(keyWord,cities[i]);
            save.addScore(citiesIndex[i]);
        }
